...
StreamFormat writeVertexStream(std::string& bin, const Stream& stream, const QuantizationPosition& qp, const QuantizationTexture& qt, const Settings& settings)
{
	...
	// copycd:: to process batchid on 3dtiles
	else if (stream.type == cgltf_attribute_type_batchid ||
	         stream.type == cgltf_attribute_type_featureid )
	{
		//int components = 1;
		// copycd::. first of all, let's save original data.
		// TODO.later, need to implement compress or quantize.
		//if (!settings.quantize)
		for (size_t i = 0; i < stream.data.size(); ++i)
		{
			const Attr& a = stream.data[i];
			...
			// for align 4byte. alloc 2-element.
			// TODO: i don't know why it can't be 2-byte.
			uint16_t v[2] = {uint16_t(a.f[0]), 0};
			bin.append(reinterpret_cast<const char*>(v), sizeof(v));
		}
